"""
Catcher Link 1: RCM 관리
ELC/TLC/ITGC RCM 등록, 업로드, 조회, 삭제
"""
...
bp_link1 = Blueprint('rcm', __name__, url_prefix='/rcm')
...
# RCM 상세 조회
@bp_link1.route('/<int:rcm_id>/view')
@login_required
def rcm_view(rcm_id):
    """RCM 상세 조회 페이지"""
    user_info = get_user_info()
...
@bp_link1.route('/process_upload', methods=['POST'])
@admin_required
...
# RCM 삭제
@bp_link1.route('/<int:rcm_id>/delete', methods=['POST'])
@admin_required
...
# RCM API - 상태 조회
@bp_link1.route('/api/<int:rcm_id>/status')
@login_required
